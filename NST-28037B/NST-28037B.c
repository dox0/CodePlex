...
#define TOUCH_XM_START	0
#define TOUCH_XP_START	1
#define TOUCH_YM_START	2
#define TOUCH_YP_START	3
#define TOUCH_PRESS_1	4
#define TOUCH_PRESS_2	5
#define TOUCH_END		6
...
extern __IO uint16_t	CursorTimer;
extern __IO uint8_t		AdcStartDelay;
...
__IO uint8_t	TouchState;
__IO uint8_t	TouchIdx;
__IO uint16_t	TouchXP,
				TouchXM,
				TouchYP,
				TouchYM,
				TouchZ1,
				TouchZ2,
				TouchXMA[20], TouchYMA[20], TouchXPA[20], TouchYPA[20];
 ...
 *	Write to LCD REG 22
 *********************************************************************/
void LcdWriteCommand22(void)
{
	LcdWriteCommand(REG_22);
}
...
{
	LCD_CS_LOW();
	LcdWriteReg16(REG_VSP, ScrollAddr);
	LCD_CS_HIGH();
}
...
{
	uint8_t row_height = FontTable[1] * FontScale;
	int h = Height();

	TextRow++;
	if (AutoScroll != 0 && Orientation == ORIENT_0)
	{
		if (TextRow * row_height >= (ScrollAddr == 0 ? h : ScrollAddr))
		{
			--TextRow;
			LcdScrollTo(row_height);
			if (ScrollAddr >= row_height)
				RectFill(0, ScrollAddr - row_height, Width(), row_height, ColorBG);
			else
				RectFill(0, ScrollAddr - row_height + h, Width(), row_height, ColorBG);
		}
	}
	DisplayAt(0, TextRow);
}
...
int Width()
{
	if (Orientation == ORIENT_0 || Orientation == ORIENT_2) return 240;
	else return 320;
}
...
int Height()
{
	if (Orientation == ORIENT_0 || Orientation == ORIENT_2) return 320;
	else return 240;
}
...
void SelectTouchPress()
{
	pinMode(TOUCH_XM, PinMode_AIn);
	pinMode(TOUCH_YP, PinMode_AIn);
	if (Orientation == ORIENT_0 || Orientation == ORIENT_1)
	{
		pinLow(TOUCH_XP);
		pinHigh(TOUCH_YM);
	}
...
void SelectTouchY()
{
	pinMode(TOUCH_YM, PinMode_AIn);
	pinMode(TOUCH_YP, PinMode_AIn);
	if (Orientation == ORIENT_0 || Orientation == ORIENT_3)
	{
		pinHigh(TOUCH_XP);
		pinLow(TOUCH_XM);
	}
...
int PutCharXY(int x, int y, char c)
{
	...
	const uint8_t * bytes = FontTable;
	uint8_t w = *bytes++;
	uint8_t h = *bytes++;
	bytes += (c - ' ') * h;
	
	LCD_CS_LOW();
	LcdSetAddress(x, y, w * FontScale, h * FontScale);
	LcdWriteCommand22();
...
{
	const uint8_t * bytes = FontTable;
	int w = (*bytes++) * FontScale;
	int h = (*bytes) * FontScale;
	int x = TextCol *  w;
	int y = TextRow * h;
	CursorOffXY(x, y, w, h);
}
...
void DisplayChar(char c)
{
	...
	if (c == '\n' || c == '\r')
	{
		...
		TextCol = 0;
	else if (TextCol < (Width() / w) && c >= ' ' && c <= 0x7E)
	{
		if (CursorState != 0)
			CursorOffXY(x, y, w, h);
...
{
	int row_height = FontTable[1] * FontScale;

	TextCol = column;
	TextRow = row;
	if (ScrollAddr != 0)
	 	TextRow = ScrollAddr / row_height - 1;
	TextRow %= (Height() / row_height);
}
...
void DisplayIdleApps()
{
	...
	if (TouchState == TOUCH_END)
	{
		...
		sz = sizeof(TouchXMA) / sizeof(uint16_t);
		
		TouchXMA[TouchIdx] = TouchXM / sz;
		TouchXPA[TouchIdx] = TouchXP / sz;
		TouchYMA[TouchIdx] = TouchYM / sz;
		TouchYPA[TouchIdx] = TouchYP / sz;
